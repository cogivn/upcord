...
export function hasPermission(permissions: string, permission: GuildPermission): boolean {
  const permissionBigInt = BigInt(permissions);
  const requiredPermission = GUILD_PERMISSIONS[permission];
  return (permissionBigInt & requiredPermission) === requiredPermission;
}
